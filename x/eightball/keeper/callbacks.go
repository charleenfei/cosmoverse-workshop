...
func (k Keeper) OnTransferAck(ctx sdk.Context, transferData transfertypes.FungibleTokenPacketData, packet channeltypes.Packet, ackSuccess bool) error {
	...
	if ackSuccess {
		...
		data, err := types.SerializeICATx(k.cdc, []proto.Message{msgSwap})
		if err != nil {
			return err
		}
		...
		_, err = k.icacontrollerKeeper.SendTx(ctx, nil, dexConnectionID, icaPortID, packetData, uint64(timeoutTimestamp))
		if err != nil {
			return err
		}
		return nil
